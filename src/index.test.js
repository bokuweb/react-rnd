...
test('should limit position by parent bounds', async t => {
  ...
  mouseMove(1000, 1000);
  t.not(rnd.childAt(0).getDOMNode().getAttribute('style').indexOf('transform: translate(700px, 500px)'), -1);
});
...
test('should limit position by selector bounds', async t => {
  ...
  mouseMove(2000, 2000);
  t.not(rnd.childAt(0).childAt(0).getDOMNode().getAttribute('style')
...
test('should get rnd updated when updateZIndex invoked', async t => {
  ...
  t.is(rnd.find('div').at(0).getDOMNode().style.zIndex, '300');
});
